...
func main() {
	...
	if len(proxyUrl) == 0 {
		...
		err := gospdyquic.ListenAndServeQuicSpdyOnly(portStr, "cert.pem", "key.pem", numOfServers, nil)
		if err != nil {
			log.Fatal(err)
		}
	} else {
		...
		err = gospdyquic.ListenAndServeQuicSpdyOnly(portStr, "cert.pem", "key.pem", numOfServers, httputil.NewSingleHostReverseProxy(parsedUrl))
		if err != nil {
			log.Fatal(err)
		}
	}
